...
class DocView {
  ...
  scrollIntoView(target) {
    ...
    let { range } = target;
    let rect = this.coordsAt(range.head, (_a = range.assoc) !== null && _a !== undefined ? _a : range.empty ? 0 : range.head > range.anchor ? -1 : 1), other;
    if (!rect)
      return;
...
class StringStream {
  ...
  indentation() {
    var _a2;
    return (_a2 = this.overrideIndent) !== null && _a2 !== undefined ? _a2 : countCol(this.string, null, this.tabSize);
  }
...
class StreamLanguage extends Language {
  ...
  getIndent(cx) {
    ...
    let start = findState(this, cx.node.tree, cx.node.from, cx.node.from, from !== null && from !== undefined ? from : cx.pos), statePos, state;
    if (start) {
      state = start.state;
    ...
    while (statePos < cx.pos) {
      ...
      if (line2.length) {
        ...
        let stream = new StringStream(line2.text, cx.state.tabSize, cx.unit, indentation < 0 ? undefined : indentation);
...
class Parse {
  constructor(lang, input, fragments, ranges) {
    ...
    let { state, tree } = findStartInFragments(lang, fragments, from, this.to, context === null || context === undefined ? undefined : context.state);
    this.state = state;
...
var identifiers = new ExternalTokenizer(identifierTokens(identifier, VariableName, callee), { contextual: true });
...
function defIDs(type, spec) {
  return (node, def) => {
    outer:
      for (let cur = node.node.firstChild, depth = 0, parent = null;; ) {
        while (!cur) {
          if (!depth)
            break outer;
          depth--;
...
var noSemicolon = new ExternalTokenizer((input, stack) => {
  ...
  if (space3.indexOf(next) > -1)
    return;
  if (next == slash3 && ((after = input.peek(1)) == slash3 || after == star))
    return;
...
var spec_identifier3 = { __proto__: null, export: 20, as: 25, from: 33, default: 36, async: 41, function: 42, in: 52, out: 55, const: 56, extends: 60, this: 64, true: 72, false: 72, null: 84, void: 88, typeof: 92, super: 108, new: 142, delete: 154, yield: 163, await: 167, class: 172, public: 235, private: 235, protected: 235, readonly: 237, instanceof: 256, satisfies: 259, import: 292, keyof: 349, unique: 353, infer: 359, asserts: 395, is: 397, abstract: 417, implements: 419, type: 421, let: 424, var: 426, using: 429, interface: 435, enum: 439, namespace: 445, module: 447, declare: 451, global: 455, defer: 471, for: 476, of: 485, while: 488, with: 492, do: 496, if: 500, else: 502, switch: 506, case: 512, try: 518, catch: 522, finally: 526, return: 530, throw: 534, break: 538, continue: 542, debugger: 546 };
...
var parser4 = LRParser.deserialize({
  ...
  specialized: [{ term: 327, get: (value) => spec_identifier3[value] || -1 }, { term: 343, get: (value) => spec_word[value] || -1 }, { term: 95, get: (value) => spec_LessThan[value] || -1 }],
  tokenPrec: 15201
});
    ...
    props: [
      ...
      /* @__PURE__ */ foldNodeProp.add({
        ...
        "JSXSelfClosingTag JSXOpenTag"(tree) {
          var _a2;
          let name2 = (_a2 = tree.firstChild) === null || _a2 === undefined ? undefined : _a2.nextSibling, close = tree.lastChild;
...
var kwCompletion2 = (name2) => ({ label: name2, type: "keyword" });
...
var autoCloseTags = /* @__PURE__ */ EditorView.inputHandler.of((view, from, to, text, defaultInsert) => {
  ...
  let closeTags = state.changeByRange((range) => {
    ...
    if (state.doc.sliceString(head - 1, head) != text || around.name == "JSXAttributeValue" && around.to > head)
      ...
      if (base3 && empty.from == head - 2 && ((name2 = elementName(state.doc, base3.firstChild, head)) || ((_a2 = base3.firstChild) === null || _a2 === undefined ? undefined : _a2.name) == "JSXFragmentTag")) {
        let insert2 = `${name2}>`;
...
function mergeAdjacent(changes, minGap) {
  for (let i2 = 1;i2 < changes.length; i2++) {
    let prev = changes[i2 - 1], cur = changes[i2];
    if (prev.toA > cur.fromA - minGap && prev.toB > cur.fromB - minGap) {
      changes[i2 - 1] = new Change(prev.fromA, cur.toA, prev.fromB, cur.toB);
      changes.splice(i2--, 1);
    }
  }
}
function normalize(a, b, changes) {
  for (;; ) {
    ...
    for (let i2 = 0;i2 < changes.length; i2++) {
      let ch = changes[i2], pre, post;
      if (pre = commonPrefix(a, ch.fromA, ch.toA, b, ch.fromB, ch.toB))
        ch = changes[i2] = new Change(ch.fromA + pre, ch.toA, ch.fromB + pre, ch.toB);
      if (post = commonSuffix(a, ch.fromA, ch.toA, b, ch.fromB, ch.toB))
        ch = changes[i2] = new Change(ch.fromA, ch.toA - post, ch.fromB, ch.toB - post);
      let lenA = ch.toA - ch.fromA, lenB = ch.toB - ch.fromB;
      if (lenA && lenB)
        continue;
      let beforeLen = ch.fromA - (i2 ? changes[i2 - 1].toA : 0);
      let afterLen = (i2 < changes.length - 1 ? changes[i2 + 1].fromA : a.length) - ch.toA;
      ...
      if (beforeLen <= text.length && a.slice(ch.fromA - beforeLen, ch.fromA) == text.slice(text.length - beforeLen)) {
        changes[i2] = new Change(ch.fromA - beforeLen, ch.toA - beforeLen, ch.fromB - beforeLen, ch.toB - beforeLen);
        moved = true;
      } else if (afterLen <= text.length && a.slice(ch.toA, ch.toA + afterLen) == text.slice(0, afterLen)) {
        changes[i2] = new Change(ch.fromA + afterLen, ch.toA + afterLen, ch.fromB + afterLen, ch.toB + afterLen);
        moved = true;
      }
    }
    if (!moved)
...
var isSurrogate1 = (code2) => code2 >= 55296 && code2 <= 56319;
var isSurrogate2 = (code2) => code2 >= 56320 && code2 <= 57343;
...
function diff(a, b, config) {
  var _a2;
  let override = config === null || config === undefined ? undefined : config.override;
  if (override)
    return override(a, b);
  scanLimit = ((_a2 = config === null || config === undefined ? undefined : config.scanLimit) !== null && _a2 !== undefined ? _a2 : 1e9) >> 1;
  timeout = (config === null || config === undefined ? undefined : config.timeout) ? Date.now() + config.timeout : 0;
  crude = false;
...
// node_modules/@fsegurai/codemirror-theme-material-dark/dist/index.js
var generalContent = {
  fontSize: "14px",
...
var base12 = "#4ebaaa";
var invalid = base08;
var darkBackground = base07;
var highlightBackground = "#2d333b30";
var background = base00;
var tooltipBackground = base01;
var selection = "#ffffff1f";
var selectionMatch = "#4A707A80";
var cursor = base04;
var activeBracketBg = "#39496650";
var activeBracketBorder = base0D;
var diagnosticWarning = base0C;
var linkColor = base0D;
var visitedLinkColor = base0F;
var addedBackground = "#1e3d2780";
var removedBackground = "#4e282880";
var addedText = "#6abf69";
var removedText = "#ff5f52";
...
var materialDarkHighlightStyle = /* @__PURE__ */ HighlightStyle.define([
  ...
  { tag: [/* @__PURE__ */ tags.function(tags.variableName)], color: base0D },
  { tag: [tags.labelName], color: base0D, fontStyle: "italic" },
  { tag: [/* @__PURE__ */ tags.definition(/* @__PURE__ */ tags.function(tags.variableName))], color: base0D },
  { tag: [/* @__PURE__ */ tags.definition(tags.variableName)], color: base0E },
  ...
  { tag: [tags.atom, tags.bool, /* @__PURE__ */ tags.special(tags.variableName)], color: base09 },
  ...
  { tag: /* @__PURE__ */ tags.definition(tags.typeName), color: base08, fontWeight: "bold" },
  ...
  { tag: /* @__PURE__ */ tags.constant(tags.name), color: base09 },
...
// src/file-editor-state.js
var SHELL_FILENAMES = new Set([".env", ".envrc", ".bashrc", ".zshrc", ".profile"]);
var SHELL_EXTENSIONS = new Set(["bash", "csh", "env", "fish", "sh", "zsh"]);
var MAX_EDITABLE_BYTES = 256 * 1024;
function fileExtensionFromName(name2) {
  if (!name2.includes("."))
    return "";
  return name2.split(".").pop();
}
function codeMirrorLanguageName(file) {
  ...
  if (mime === "text/css")
    return "css";
  const name2 = basename(file).toLowerCase();
  if (name2 === "dockerfile")
    return "dockerfile";
  if (name2 === "makefile" || name2 === "justfile")
    return "makefile";
  if (SHELL_FILENAMES.has(name2))
    return "shell";
  const extension = fileExtensionFromName(name2);
  if (["js", "mjs", "cjs", "jsx"].includes(extension))
    return "javascript";
  if (["ts", "mts", "cts", "tsx"].includes(extension))
    return "typescript";
  if (["json", "jsonc", "ipynb"].includes(extension))
    return "json";
  if (["md", "markdown", "mdx"].includes(extension))
    return "markdown";
  if (["html", "htm", "astro", "svelte", "vue"].includes(extension))
    return "html";
  if (["css", "less", "scss"].includes(extension))
    return "css";
  if (extension === "go")
    return "go";
  ...
  mount(options) {
    ...
    this.view = new EditorView({
      ...
      state: EditorState.create({
        doc: options.content,
        extensions: editorExtensions(options.file, !!options.readOnly, options.originalContent ?? options.content, this.saveKeymap, this.changeListener)
      })
    });
  }
  createSearchToolbar() {
...
function codeMirrorLanguageExtension(file) {
  const name2 = (file.path || "").split("/").pop()?.toLowerCase() || "";
...
function editorExtensions(file, readOnly2, originalContent, ...extra) {
  const changeIndicator = readOnly2 ? [] : gitChangeGutter(originalContent);
  return [
    lineNumbers(),
...
function buildGitChangeMarkers(originalContent, doc2) {
  const builder = new RangeSetBuilder;
  for (const change of diff(originalContent, doc2.toString())) {
    if (change.fromB === change.toB) {
      addLineMarker(builder, doc2, change.fromB, "deleted");
    } else {
      const kind = change.fromA === change.toA ? "added" : "modified";
      addLineMarkers(builder, doc2, change.fromB, change.toB, kind);
    }
  }
  return builder.finish();
}
function addLineMarkers(builder, doc2, from, to, kind) {
  const docEnd = doc2.length;
  const start = Math.max(0, Math.min(from, docEnd));
  const end = Math.max(start, Math.min(to, docEnd));
  if (start === end) {
    addLineMarker(builder, doc2, start, kind);
    return;
  }
  const firstLine = doc2.lineAt(start).number;
  const lastLine = doc2.lineAt(Math.max(start, end - 1)).number;
  for (let lineNumber = firstLine;lineNumber <= lastLine; lineNumber++) {
    const line = doc2.line(lineNumber);
    builder.add(line.from, line.from, gitMarkers[kind]);
  }
}
function addLineMarker(builder, doc2, position, kind) {
  const safePosition = Math.max(0, Math.min(position, doc2.length));
  const line = doc2.lineAt(safePosition);
  builder.add(line.from, line.from, gitMarkers[kind]);
}
...
function materialThemeIcon(name2, size = 16) {
...
function ensureOutsideClose(app) {
  if (app.fileBrowserOutsideCloseBound)
    return;
  document.addEventListener("pointerdown", (event) => {
    ...
    if (target.closest?.("[data-file-browser-sidebar]"))
      return;
    if (target.closest?.("[data-file-editor-modal]"))
      return;
    if (target.closest?.(`[data-plugin-toolbar-button="${PANEL_ID}"]`))
      return;
...
function syncSidebarTop(app, sidebar = app.querySelector("[data-file-browser-sidebar]")) {
  ...
  const top2 = topbar ? Math.max(0, topbar.getBoundingClientRect().bottom) : 0;
  sidebar.style.setProperty("--file-browser-sidebar-top", `${top2}px`);
}
...
function renderTree(panel, state) {
  const rows = [];
  for (const node of state.files)
    appendNode(rows, node, state, 0);
...
function appendNode(rows, node, state, depth) {
  ...
  for (const child of children)
    appendNode(childRows, child, state, depth + 1);
...
function rowNode(node, state, depth, expanded) {
  ...
  row.className = ["tree-node", isDir ? "dir" : "file", status, state.selectedPath === path ? "selected" : ""].filter(Boolean).join(" ");
  ...
  name2.textContent = node.name || path;
  main.append(fileIcon, name2);
  row.append(main);
  if (status !== "clean") {
...
function materialIconName(node, isDir, expanded) {
  ...
  const name2 = String(node.name || node.path || "").toLowerCase();
  const exact = {
    "package.json": "nodejs",
...
function handleAction(context, state, panel, target) {
  ...
  if (action === "refresh")
    return refresh(context, state, panel);
  if (action === "toggle") {
    state.query = "";
    const search2 = panel.querySelector("[data-file-browser-search]");
    if (search2)
      search2.value = "";
  ...
  if (action === "root-menu")
    return showActionMenu(context, state, panel, target, { path: "", kind: "root" });
  if (action === "row-menu")
    return showActionMenu(context, state, panel, target, { path, kind: target.dataset.kind });
  if (action === "new-file")
    return createFile(context, state, panel, target.dataset.parent || "");
  if (action === "upload")
    return uploadFile(context, state, panel, target.dataset.parent || "");
  if (action === "rename")
    return renamePath(context, state, panel, path);
  if (action === "delete")
    return deletePath(context, state, panel, path);
  return;
}
async function openEditor(context, path) {
  ...
  try {
    ...
    if (editor.dataset.workspaceId !== workspaceId || editor.dataset.path !== path)
      return;
...
async function mountCodeMirrorEditor(context, editor, file) {
  ...
  editor.fileEditor = new CodeMirrorFileEditor(host, {
    ...
    onChange(content2) {
      save.disabled = content2 === editor.dataset.cleanContent;
    },
    onSave() {
      saveEditor(context.app, editor);
    }
  });
...
async function saveEditor(app, editor) {
  ...
  const content2 = editor.fileEditor?.getValue?.() || "";
  if (!workspaceId || !path || !pluginContext)
    return;
  save.disabled = true;
  status.textContent = "saving…";
  try {
    const file = await pluginContext.backend("write", { workspaceId, data: { path, content: content2 } });
    editor.dataset.cleanContent = file.content || content2;
...
function closeEditor(editor) {
  const content2 = editor.fileEditor?.getValue?.() || "";
  if (content2 !== editor.dataset.cleanContent && !window.confirm("Discard unsaved file changes?"))
    return;
  destroyEditorInstance(editor);
  editor.hidden = true;
}
function showActionMenu(context, state, panel, target, item) {
  ...
  menu.append(menuButton("new-file", "new file", parent), menuButton("upload", "upload file", parent));
  if (item.kind !== "root") {
...
async function renamePath(context, state, panel, path) {
  ...
  if (!workspaceId || !path || !next || next === path)
    return;
...
async function deletePath(context, state, panel, path) {
  const workspaceId = context.app.dataset.activeWorkspaceId;
  if (!workspaceId || !path || !window.confirm(`Delete ${path}?`))
    return;
...
function joinPath(parent, name2) {
  return parent ? `${parent.replace(/\/$/, "")}/${name2}` : name2;
}
